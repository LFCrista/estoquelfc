
import { NextResponse } from "next/server";
import { createClient } from "@supabase/supabase-js";

const supabase = createClient(
	process.env.NEXT_PUBLIC_SUPABASE_URL!,
	process.env.SUPABASE_SERVICE_ROLE_KEY!
);

export async function POST(req: Request) {
	const { produto_id, prateleira_id } = await req.json();
	const { error } = await supabase
		.from("estoque")
		.insert([{ produto_id, prateleira_id }]);
	if (error) return NextResponse.json({ error: error.message }, { status: 400 });
	return NextResponse.json({ message: "Estoque cadastrado com sucesso" });
}

export async function PATCH(req: Request) {
	const { id, produto_id, prateleira_id } = await req.json();
	const { error } = await supabase
		.from("estoque")
		.update({ produto_id, prateleira_id })
		.eq("id", id);
	if (error) return NextResponse.json({ error: error.message }, { status: 400 });
	return NextResponse.json({ message: "Estoque atualizado com sucesso" });
}

export async function GET(req: Request) {
	const { searchParams } = new URL(req.url);
	const page = parseInt(searchParams.get("page") || "1", 10);
	const limit = parseInt(searchParams.get("limit") || "100", 10);
	const from = (page - 1) * limit;
	const to = from + limit - 1;

	// Filtros opcionais
	const search = searchParams.get("search")?.trim() || "";
	const searchField = searchParams.get("searchField") || "produto_id";

	let query = supabase
		.from("estoque")
		.select("id, produto_id, prateleira_id", { count: "exact" })
		.order("id", { ascending: true });

	if (search) {
		if (["produto_id", "prateleira_id"].includes(searchField)) {
			query = query.ilike(searchField, `%${search}%`);
		}
	}

	query = query.range(from, to);

	const { data, error, count } = await query;

	if (error) return NextResponse.json({ error: error.message }, { status: 400 });
	return NextResponse.json({ estoque: data, total: count });
}
